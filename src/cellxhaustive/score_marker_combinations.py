...
def score_marker_combinations(
    ...
    nb_phntp: array(float)
      ...
      positive and negative markers from 'markers_comb') identified in
      'mat_comb' across grid composed of metrics 'x_samplesxbatch_space' in D0
    ...
    nb_undef_cells = np.zeros((
        len(x_samplesxbatch_space),
    ...
    for phenotype in np.unique(phntp_per_cell):
        ...
        for batch in np.unique(batches_label):
            ...
            keep_phenotype_batch = (
                cell_count_sample[:, np.newaxis] >= y_cellxsample_space
            )
            ...
            # - 'np.sum(keep_phenotype_batch, axis=0)' calculates number of
            # samples satisfying cell/sample threshold for a given y in grid

            # Check whether previous proportions satisfy sample/batch threshold
            keep_phenotype_batch = (
                keep_phenotype_batch >= x_samplesxbatch_space[:, np.newaxis]
            )
            ...
            keep_phenotype = np.logical_and(
                keep_phenotype, keep_phenotype_batch
            )
            # Note: for consistency, phenotypes have to be present in all
            # batches, hence usage of 'np.logical_and()'

        # Add 'phenotype' presence/absence to phenotype counter
        nb_phntp += keep_phenotype * 1

        # Add number of undefined cells to counter
        nb_undef_cells += np.logical_not(keep_phenotype) * np.sum(
            phntp_per_cell == phenotype
        )
