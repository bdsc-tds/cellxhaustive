...
def main():
    ...
    logging.info(
        "Gating markers and extracting associated data in each cell type"
    )
    ...
    with open(marker_file_path, "w") as file:
        ...
        for label, is_label, markers_interest, mask_interest_inv in zip(
            uniq_labels,
            is_label_lst,
            ...
            markers_rep_batches = []
            label_unique_batch = np.unique(
                batches[is_label]
            )  # Get batch names in 'label' cells
            nb_batch = len(
                label_unique_batch
            )  # Get number of batch in 'label' cells
            for batch in label_unique_batch:
                ...
                try:
                    mat_subset = mat[is_label_batch, :][
                        :, mask_interest_inv
                    ]  # With markers of interest
                except NameError:
                    mat_subset = mat[
                        is_label_batch, :
                    ]  # Without markers of interest
                logging.debug("\t\t\tEvaluating marker bimodality")
                marker_center_values = np.abs(
                    np.mean(mat_subset, axis=0) - two_peak_threshold
                )
                marker_threshold_value = np.sort(marker_center_values)[
                    max_markers - 1
                ]  # Select max_markers-th center value (in ascending order)
                # Note: '- 1' is used to compensate 0-based indexing
                # Select markers with center smaller than max_markers-th center
                is_center_smaller = (
                    marker_center_values <= marker_threshold_value
                )
                try:
                    markers_rep = markers[mask_interest_inv][
                        is_center_smaller
                    ]  # With markers of interest
                except NameError:
                    markers_rep = markers[
            ...
            if (
                nb_batch == 1
            ):  # No need to filter markers if there is only 1 batch
                logging.info(
                    "\t\tOnly one batch in total, no need to filter markers"
                )
                markers_representative, missing_markers, nb_batch_end = (
                    markers_rep_batches,
                    [],
                    1,
                )
                ...
                logging.info(
                    f"\t\t{nb_batch} batches in total. Selecting markers present in all batches"
                )
                markers_representative, missing_markers, nb_batch_end = (
                    get_repr_markers(markers_rep_batches, nb_batch)
                )
            ...
            file.write(
                f"\tMarkers kept (found in {nb_batch_end} batches): {mk_rep_str}\n"
            )
            file.write(
                f"\tMarkers removed (found in less than {nb_batch_end} batches): {missing_mk_str}\n\n"
            )
            ...
            if detection_method_lst[label_idx] != "auto":
                if detection_method_lst[label_idx] > len(
                    markers_representative
                ):
                    logging.error(
                        f"\t\tNumber of markers kept is less than detection_method value for cell type <{label}>"
                    )
                    logging.error(
                        f"\t\tNumber of representative markers: {len(markers_representative)}"
                    )
                    logging.error(
                        f"\t\tRequested combination length: {detection_method_lst[label_idx]}"
                    )
                    logging.error(
                        "\t\tTry to decrease <detection_method> value for this cell type or use <auto> mode"
                    )
                    sys.exit(1)

            # Extract expression, batch and sample information across all
            # batches for cell type 'label'
            logging.info(
                "\t\tExtracting matching expression, batch and sample information"
            )
            ...
            markers_representative_lst.append(
                markers_representative
            )  # Store representative markers
            # Note: keeping data in list of arrays makes multiprocessing easier
            # and reduce overall memory usage
