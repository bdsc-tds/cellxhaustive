...
def get_default_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Package to annotate cell types using \
            CITE-seq ADT data."
    )
    ...
    parser.add_argument(
        ...
        dest="input_path",
        type=str,
        help="Path to input table with expression data and samples, batch, and \
    ...
    parser.add_argument(
        ...
        dest="marker_path",
        type=str,
        help="Path to file with list of markers to consider during analyses",
        required=True,
    )
    parser.add_argument(
        "-o",
    ...
    parser.add_argument(
        ...
        dest="max_markers",
        type=int,
        help="Maximum number of markers to select among total list of markers. \
            Must be an integer less than or equal to number of markers in \
            total list provided with '-m' parameter [15]",
        required=False,
        default=15,
    )
    parser.add_argument(
        "-mi",
        "--markers-interest",
        dest="markers_interest",
        type=str,
        help="Comma-separated list of markers of interest that will appear \
            in final combination. Global setting that applies to all cell \
            types []",
        required=False,
        default="",
    )
    parser.add_argument(
        ...
        type=str,
        help="Method to identify length of best marker combination. Must be \
            'auto' to use default algorithm or an integer to set combination \
            length. Integer must be less than '-a' parameter. Global setting \
            that applies to all cell types [auto]",
        required=False,
        default="auto",
    )
    parser.add_argument(
        ...
        dest="config_path",
        type=str,
        help="Path to config file with cell type-specific detection method and \
            markers of interest []",
        required=False,
        default="",
    )
    ...
    parser.add_argument(
        ...
        dest="log_level",
        type=str,
        help="Verbosity level of log file. Must be 'debug', 'info' or \
            'warning' [info]",
        required=False,
        default="info",
    ...
    parser.add_argument(
        "-e",
        "--three-peaks",
        dest="three_peak_markers",
        type=str,
        help="Path to file with markers with three peaks or comma separated \
            list of markers with three peaks []",
        required=False,
        default="",
    )
    parser.add_argument(
        ...
        dest="thresholds",
        type=str,
        help="Comma-separated list of 3 floats defining thresholds to \
            determine whether marker are negative or positive. 1st number is \
            for two peaks markers, last 2 numbers are for three peaks markers \
            [3,2,4]",
        required=False,
        default="3,2,4",
    )
    parser.add_argument(
        "-q",
        "--min-samplesxbatch",
        dest="min_samplesxbatch",
        type=float,
        help="Minimum proportion of samples within each batch with at least \
            'min_cellxsample' cells for a new annotation to be considered. \
            Must be a float in [0.01; 1] [0.5]",
        required=False,
        default=0.5,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-p",
        "--knn-min-probability",
        dest="knn_min_probability",
        type=float,
        help="Confidence threshold for KNN classifier to reassign a new cell \
            type to previously undefined cells. Must be a float in \
            [0; 1] [0.5]",
        required=False,
        default=0.5,
    )
    parser.add_argument(
        ...
        dest="cores",
        type=int,
        help="Number of cores to use. Specifying more than one core will run \
            parallel jobs which will increase speed. Must be a strictly \
            positive integer [1]",
        required=False,
        default=1,
    )
...
# Function to check status of 'detection_method' parameter; used in
# parse_config_file()
def get_detection_method(detection_method, offset):
    ...
    try:  # Integer was provided
        ...
        logging.info(
            "{offset}Limiting combinations to {method} markers".format(
        ...
        if detection_method == "auto":  # Default setting was provided
            logging.info(
                "{offset}Using default algorithm".format(offset="\t" * offset)
            )
            method = "auto"
        else:
            logging.error(
                "{offset}Unknown detection method. Please provide <auto> or an integer superior or equal to 2".format(
                    offset="\t" * offset
                )
            )
            sys.exit(1)
...
# Function to parse config file and return parameter values; used in
# cellxhaustive.py
def parse_config_file(
    config_path, uniq_labels, markers_interest, detection_method
):
    """
    ...
    try:
        ...
        if config_dict is None:
            logging.error(
                "\tConfig file is empty. Plese fill it or do not use it in the command line"
            )
            sys.exit(1)
        ...
        if markers_interest:  # With markers in CLI
            markers_interest = np.array(
                markers_interest.split(","), dtype="str"
            )
            mk_int_str = ", ".join(markers_interest)
            logging.info(
                f"\t\t\tFound {len(markers_interest)} markers: {mk_int_str}"
            )
        else:  # # Without markers in CLI
            logging.warning(
                "\t\t\tNo markers provided in CLI, using default empty array"
            )
        ...
        try:  # Information on markers of interest in config file
            ...
            if (
                markers_dict is None
            ):  # User probably forgot to delete field, use default
                logging.warning(
                    "\t\t\t<markers_interest> field is present but empty. Using default empty array"
                )
                markers_interest = np.empty(0, dtype="str")
                logging.info("\t\t\tPropagating this setting to all cell types")
                markers_interest_lst = [
                    markers_interest for _ in range(cell_pop_nb)
                ]
            elif "all" in markers_dict.keys():
                if (
                    markers_dict["all"] is None
                ):  # User probably forgot to delete field, use default
                    logging.warning(
                        "\t\t\t<markers_interest: all> field is present but empty. Using default empty array"
                    )
                    markers_interest = np.empty(0, dtype="str")
                else:
                ...
                for label in uniq_labels:
                    if label in markers_dict.keys():
                        if (
                            markers_dict[label] is None
                        ):  # User probably forgot to delete field, use default
                            logging.warning(
                                f"\t\t\t\t<markers_interest: all: {label}> field is present but empty. Using default empty array"
                            )
                            markers_interest_lst.append(
                                np.empty(0, dtype="str")
                            )
                        else:
                            mk_ct_str = ", ".join(markers_dict[label])
                            logging.info(
                                f"\t\t\t\tFound markers for cell type <{label}>: {mk_ct_str}"
                            )
                            markers_interest_lst.append(
                                np.array(markers_dict[label])
                            )
                    else:
                        logging.warning(
                            f"\t\t\t\tNo markers found for cell type <{label}>. Using default empty array"
                        )
                        markers_interest_lst.append(np.empty(0, dtype="str"))
        except KeyError:  # No information on markers of interest in config file
            logging.warning(
                "\t\t\tNo markers settings provided, using default empty array"
            )
            markers_interest = np.empty(0, dtype="str")
            logging.info("\t\t\tPropagating this array to all cell types")
            markers_interest_lst = [
        ...
        try:  # Information on detection method in config file
            ...
            if (
                detection_dict is None
            ):  # User probably forgot to delete field, use default
                logging.warning(
                    "\t\t\t<detection_method> field is present but empty. Using default algorithm"
                )
                detection_method_lst = ["auto"] * cell_pop_nb
            elif "all" in detection_dict.keys():
                if (
                    detection_dict["all"] is None
                ):  # User probably forgot to delete field, use default
                    logging.warning(
                        "\t\t\t<detection_method: all> field is present but empty. Using default algorithm"
                    )
                ...
                for label in uniq_labels:
                    if label in detection_dict.keys():
                        if (
                            detection_dict[label] is None
                        ):  # User probably forgot to delete field, use default
                            logging.warning(
                                f"\t\t\t\t<detection_method: all: {label}> field is present but empty. Using default algorithm"
                            )
                            detection_method_lst.append("auto")
                        else:
                            logging.info(
                                f"\t\t\t\tFound detection method for cell type <{label}>: {detection_dict[label]}"
                            )
                            detection_method = get_detection_method(
                                detection_dict[label], 5
                            )
                            detection_method_lst.append(detection_method)
                    else:
                        logging.info(
                            f"\t\t\t\tNo detection method found for cell type <{label}>. Using default algorithm"
                        )
                        detection_method_lst.append("auto")
        except KeyError:  # No information on detection method in config file
            logging.warning(
                "\t\t\tNo detection method settings provided, using default algorithm"
            )
            detection_method = "auto"
            logging.info("\t\t\tPropagating this setting to all cell types")
            detection_method_lst = [detection_method] * cell_pop_nb
...
def validate_cli(args):
    ...
    try:
        markers = (
            pd.read_csv(args.marker_path, sep="\t", header=None)
            .to_numpy(dtype=str)
            .flatten()
        )
    except FileNotFoundError:
        logging.error(
            f"\tCould not find <{args.marker_path}>. Please double-check file path"
        )
        sys.exit(1)
    ...
    try:
        input_table = pd.read_csv(
            args.input_path, sep="\t", header=0, index_col=0
        )
    except FileNotFoundError:
        logging.error(
            f"\tCould not find <{args.input_path}>. Please double-check file path"
        )
        sys.exit(1)
    except Exception as e:
        logging.error(f"\t{e}")
    ...
    try:
        ...
        logging.error(
            f"\tCould not find marker <{e}> in <{args.input_path}>. Please double-check marker list"
        )
        sys.exit(1)
    except Exception as e:
        logging.error(f"\t{e}")
    ...
    try:
        ...
        logging.warning(
            "\tSetting cell type value to <cell_type0> for all cells"
        )
        cell_labels = np.full(input_table.shape[0], "cell_type0")
    else:
        cell_labels = input_table.iloc[:, cell_type_idx].to_numpy(dtype=str)
        labels_array = np.unique(cell_labels)
        logging.info(f"\tFound {len(labels_array)} cell types: {labels_array}")
    ...
    try:
        logging.info(
            f"Checking for 3 peaks markers file at <{args.three_peak_markers}>"
        )
        ...
        if args.three_peak_markers:
            three_peak_markers = list(
                filter(None, args.three_peak_markers.split(","))
            )
            three_peak_markers = np.array(three_peak_markers, dtype="str")
            mk_3p_str = ", ".join(three_peak_markers)
            logging.info(
                f"\t\tFound {len(three_peak_markers)} markers: {mk_3p_str}"
            )
        else:
            logging.warning(
                "\t\tNo markers provided in CLI, using default empty array"
        ...
        logging.info(
            f"\tFound {len(three_peak_markers)} markers in <{three_peak_markers}>"
        )
    ...
    try:
        if len(three_peak_markers) == 0:  # Only two peaks markers
            two_peak_threshold = [float(x) for x in args.thresholds.split(",")][
                0
            ]
            three_peak_low = None  # Not needed so set to None
            three_peak_high = None  # Not needed so set to None
        else:  # Some three peaks markers
            two_peak_threshold, three_peak_low, three_peak_high = [
                float(x) for x in args.thresholds.split(",")
            ]
    except ValueError as e:
        logging.error(f"\t{e}")
        logging.error(
            "Please make sure you provide 1 or 3 numbers separated by comma"
        )
        sys.exit(1)
    except Exception as e:
        logging.error(f"\t{e}")
    ...
    try:
        ...
        logging.warning(
            f"\t\tCould not find <{args.cell_type_path}>. Loading default file"
        )
        ...
        default_config_file = (
            imp_resources.files(config) / "major_cell_types.yaml"
        )
    ...
    logging.info(
        "\tChecking presence of markers of interest in general marker list"
    )
    ...
    for label, markers_interest in zip(uniq_labels, markers_interest_lst):
        if sum(np.isin(markers_interest, markers)) == len(markers_interest):
            ...
            logging.info(
                f"\t\tAll markers of interest located for cell type <{label}>"
            )
        else:
            missing_interest = markers_interest[
                np.isin(markers_interest, markers, invert=True)
            ]
            logging.error(
                f"\t\tPlease double-check markers of interest for cell type <{label}>"
            )
            mk_missing_str = ", ".join(missing_interest)
            logging.error(
                f"\t\tSome markers are missing in general marker list: {mk_missing_str}"
            )
            sys.exit(1)
    ...
    for label, detection_method in zip(uniq_labels, detection_method_lst):
        if isinstance(detection_method, int):  # Only process integers
            if detection_method > len(markers):
                logging.error(
                    f"\t<-dm/--detection-method> for cell type <{label}> must be lower than number of markers in <{args.marker_path}>"
                )
                sys.exit(1)
            elif detection_method < len(markers_interest):
                logging.error(
                    f"\t<-dm/--detection-method> for cell type <{label}> must be higher than number of markers in {markers_interest}"
                )
                sys.exit(1)
            else:
                logging.info(
                    f"\t\tValid detection method value for cell type <{label}>"
                )

    logging.info("\tChecking other parameters")
    if not (1 <= args.max_markers <= len(markers)):
        logging.error(
            f"\t<-a/--max-markers> must be an integer between 1 and {len(markers)}"
        )
        sys.exit(1)
    if not (0.01 <= args.min_samplesxbatch <= 1):
        logging.error(
            "\t<-q/--min-samplesxbatch> must be a float between 0.01 and 1"
        )
        sys.exit(1)
    if not (1 <= args.min_cellxsample <= 100):
        logging.error(
            "\t<-r/--min-cellxsample> must be an integer between 1 and 100"
        )
        sys.exit(1)
    if not (0 <= args.knn_min_probability <= 1):
        logging.error(
            "\t<-p/--knn-min-probability> must be a float between 0 and 1"
        )
        sys.exit(1)
