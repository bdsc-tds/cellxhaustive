...
def assign_cell_types(cell_name,
                      best_comb_name,
                      mat_representative,
                      batches_label,
                      samples_label,
    ...
    logging.info(f'\t\t\t\t\t{cell_name} - ({best_comb_name}): Trimming cell classification to keep only relevant markers')
    ...
    logging.info(f'\t\t\t\t\t{cell_name} - ({best_comb_name}): Reducing redundancy in new classification')
    ...
    logging.info(f'\t\t\t\t\t{cell_name} - ({best_comb_name}): Determining exact matches between phenotypes and cell classification')
    phntp_match = []
    cell_types_match = []
    ...
    if len(phntp_match) == 0:  # No exact match
        ...
        base_name = cell_name
        logging.info(f'\t\t\t\t\t\t{cell_name} - ({best_comb_name}): No exact match between phenotypes and cell classification')

    elif len(phntp_match) == 1:  # One exact match that was already determined
        base_comb = phntp_match[0]
        base_name = cell_types_match[0]
        logging.info(f'\t\t\t\t\t\t{cell_name} - ({best_comb_name}): 1 exact match between phenotypes and cell classification')

    else:  # Several exact matches that were already determined
        base_comb = phntp_match
        base_name = cell_types_match
        logging.info(f'\t\t\t\t\t\t{cell_name} - ({best_comb_name}): Found {len(base_name)} exact match between phenotypes and cell classification')

    # Get mapping dictionary to convert names
    logging.info(f'\t\t\t\t\t{cell_name} - ({best_comb_name}): Building dictionary to convert cell type names')
    ...
    logging.info(f'\t\t\t\t\t{cell_name} - ({best_comb_name}): Renaming cell types')
